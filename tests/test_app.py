...
@pytest.fixture
def app_instance():
    """Provides a FigmaApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return FigmaApp(integration=mock_integration)

def test_universal_mcp_figma_app_initialization(app_instance):
    """
    Test that the FigmaApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "figma", "FigmaApp instance has unexpected name."


def test_universal_mcp_figma_tool_docstrings_format(app_instance):
    """
    Test that each tool method in FigmaApp has a well-formatted docstring,
...
def test_universal_mcp_figma_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in FigmaApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
